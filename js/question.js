...
const question = (page) => {
  ...
  fetch(`https://quizzone-server.onrender.com/question/list/?page=${pageNumber}`)
    .then((res) => res.json())
    .then((data) => {
...
const displayQuestionAndAnswer = (item) => {
  ...
  div1.innerHTML = `
    ...
    `;
  fetch(`https://quizzone-server.onrender.com/question/answer_option/?question=${item.id}`)
    .then((res) => res.json())
    .then((data) => {
      data.forEach((answer) => {
        const answer_op = document.getElementById("answer_op");
...
const next = (question_category) => {
  fetch(`https://quizzone-server.onrender.com/question/list/`)
    .then((res) => res.json())
    .then((data) => {
...
const questionPaperSubmit = () => {
  ...
  fetch(`https://quizzone-server.onrender.com/authentication/user/?user_id=${user_id}`)
    .then((res) => res.json())
    .then((data) => {
      user = data[0].id;
      formData.append("user", user);
      formData.append("mark", all_mark);
      fetch("https://quizzone-server.onrender.com/question/scriptSubmission/", {
        method: "POST",
        body: formData,
      })
        .then((res) => res.json())
